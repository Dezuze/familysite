  ...
  <div class="min-h-screen bg-[#F5F5F7] text-slate-900 pt-32 pb-16 px-4 flex flex-col items-center font-sans">
    ...
    <div class="max-w-3xl w-full">
        ...
        <div class="flex items-center justify-center mb-10 gap-4">
             ...
             <div class="h-0.5 w-12 bg-slate-200"></div>
             <div :class="['w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-300', step >= 3 ? 'bg-slate-900 text-white shadow-lg scale-110' : 'bg-white text-slate-400 border border-slate-200']">3</div>
        </div>
            ...
            <div class="relative z-10 transition-all duration-500" v-auto-animate>
                ...
                <form @submit.prevent="saveProfile" class="space-y-4">
                    ...
                    <div v-if="step === 1" class="space-y-8 animate-fade-in">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="group">
                                ...
                                <input v-model="form.first_name" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3.5 text-slate-900 font-medium focus:bg-white focus:border-brand-olive focus:ring-4 focus:ring-brand-olive/10 outline-none transition-all placeholder:text-slate-400" placeholder="John">
                            </div>
                            <div class="group">
                                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 ml-1">Last Name</label>
                                <input v-model="form.last_name" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3.5 text-slate-900 font-medium focus:bg-white focus:border-brand-olive focus:ring-4 focus:ring-brand-olive/10 outline-none transition-all placeholder:text-slate-400" placeholder="Doe">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    <select v-model="form.blood_group" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3.5 text-slate-900 font-medium focus:bg-white focus:border-brand-olive focus:ring-4 focus:ring-brand-olive/10 outline-none transition-all appearance-none cursor-pointer">
                         ...
                         <div class="group">
                             ...
                             <div class="space-y-4">
                                 ...
                                 <div v-if="form.parents.length > 0" class="flex flex-wrap gap-2">
                                     <div v-for="pid in form.parents" :key="pid" class="flex items-center gap-2 bg-brand-olive/5 text-brand-olive px-3 py-1.5 rounded-full text-xs font-bold border border-brand-olive/10">
                                         {{ getMemberName(pid) }}
                    ...
                    <div class="pt-10 flex justify-between items-center">
                        ...
                        <button v-else type="submit" :disabled="loading" class="bg-linear-to-b from-brand-olive to-brand-slate hover:brightness-110 text-white px-8 py-3.5 rounded-2xl font-bold shadow-xl shadow-brand-olive/20 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div v-if="step === 3" class="space-y-8 animate-fade-in">
                        ...
                        <div v-if="managedMembers.length > 0" class="grid grid-cols-1 gap-4">
                            <div v-for="m in managedMembers" :key="m.id" class="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-center justify-between group hover:border-brand-olive/50 transition-all">
                                ...
                                <div class="flex items-center gap-2">
                                    <button @click="editManagedMember(m)" type="button" class="p-2 text-slate-400 hover:text-brand-olive hover:bg-brand-olive/5 rounded-lg transition-all">
                        ...
                        <div class="pt-6 flex justify-between items-center">
                            ...
                            <button @click="router.push('/familytree')" type="button" class="bg-slate-900 text-white px-8 py-3 rounded-2xl font-bold shadow-lg shadow-slate-900/20 transition-all hover:scale-[1.02]">
                                Finish & View Tree
                            </button>
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="saveManagedMember" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input v-model="managedForm.first_name" required type="text" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Last Name</label>
                                <input v-model="managedForm.last_name" required type="text" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select v-model="managedForm.relation" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                                    <option>Child</option>
                            ...
                            <div>
                                ...
                                <select v-model="managedForm.gender" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                                    <option value="M">Male</option>
                                    <option value="F">Female</option>
                                    <option value="O">Other</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input v-model="managedForm.date_of_birth" type="date" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                            </div>
                             <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Blood Group</label>
                                <select v-model="managedForm.blood_group" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                                    <option>Unknown</option><option>A+</option><option>A-</option><option>B+</option><option>B-</option><option>O+</option><option>O-</option><option>AB+</option><option>AB-</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input v-model="managedForm.occupation" type="text" placeholder="e.g. Student at Model School" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all">
                        </div>

                         <div>
                             <label class="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Bio</label>
                             <textarea v-model="managedForm.bio" rows="2" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-900 focus:bg-white focus:border-brand-olive outline-none transition-all resize-none"></textarea>
                        </div>
                        ...
                        <div class="pt-6 flex gap-3">
                            ...
                            <button type="submit" :disabled="loading" class="flex-1 bg-brand-olive hover:bg-brand-olive/90 text-white px-8 py-3.5 rounded-2xl font-bold shadow-lg shadow-brand-olive/20 transition-all active:scale-95 disabled:opacity-50">
      ...
      <div v-if="showCropper" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div class="bg-white rounded-3xl overflow-hidden w-full max-w-lg shadow-2xl animate-fade-up">
              ...
              <div class="p-4 flex gap-3 justify-end bg-white border-t border-gray-100">
                  ...
                  <button @click="cropImage" class="px-6 py-2.5 rounded-xl font-bold text-white bg-brand-olive hover:bg-brand-olive/90 shadow-lg shadow-brand-olive/30 transition-all active:scale-95">Set Profile Photo</button>
              </div>
          </div>
      </div>
    </Teleport>
