<template>
  <div class="min-h-screen bg-slate-200/50 text-slate-800 font-sans pt-32 relative overflow-hidden">
       ...
       <div class="flex flex-col items-end gap-4 pointer-events-auto">
          ...
          <div class="relative">
             <input v-model="searchQuery" @keyup.enter="performSearch" type="search" placeholder="Find member..." 
                    class="pl-10 pr-4 py-2 rounded-lg bg-white/90 backdrop-blur border border-slate-300 text-slate-900 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold shadow-md w-48 md:w-64 transition-all">
          ...
          <div class="bg-white/90 backdrop-blur rounded-lg p-1 flex border border-slate-300 shadow-md">
             <button 
                ...
                :class="['px-4 py-2 rounded-md text-sm font-bold transition-all', viewMode==='visual' ? 'bg-brand-olive text-white shadow-lg' : 'text-slate-500 hover:text-brand-olive']"
             >
               Visual
             </button>
             <button 
                @click="viewMode = 'grid'"
                :class="['px-4 py-2 rounded-md text-sm font-bold transition-all', viewMode==='grid' ? 'bg-brand-olive text-white shadow-lg' : 'text-slate-500 hover:text-brand-olive']"
             >
               Directory
             </button>
          </div>
    ...
    <div v-show="viewMode === 'visual'" class="w-full h-[calc(100vh-100px)] cursor-move" ref="chartContainer">
     ...
     <!-- Grid View -->
     <div v-if="viewMode === 'grid'" class="max-w-7xl mx-auto px-4 pt-4 pb-20 overflow-y-auto h-[calc(100vh-100px)]">
         
         <!-- Directory Controls (only shown in grid view) -->
         <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8 bg-white p-4 rounded-xl border border-slate-200 shadow-md animate-fade-in">
           <div class="flex items-center gap-4 text-sm text-slate-500 font-medium ml-auto">
             <div class="flex items-center gap-2">
                 <label>Layout</label>
                 <select v-model="layout" class="bg-slate-50 border border-slate-300 rounded px-2 py-1 text-slate-800 focus:outline-none focus:ring-1 focus:ring-brand-gold cursor-pointer">
             ...
             <div v-if="layout === 'grid' || layout === 'compact'" class="hidden sm:flex items-center gap-2">
               <label>Size</label>
               <input type="range" min="160" max="420" v-model.number="minWidth" class="w-24 accent-brand-gold cursor-pointer" />
             </div>
           </div>
         </div>
...
// --- D3 Logic ---
   const initGraph = () => {
      ...
      function renderCard(selection: d3.Selection<any, any, any, any>, dx=0, d: FamilyMember) {
          if (!d) return
          const cardWidth = 140
          ...
          group.append("rect")
            ...
            .attr("fill", isUser ? "#F9F9F7" : "#ffffff")
            .attr("stroke", isUser ? "#CDAA7D" : "#cbd5e1")
          ...
          group.append("image")
            .attr("xlink:href", resolveImage(d.photo || null) || `https://ui-avatars.com/api/?name=${d.name}&background=f1f5f9&color=64748b`)
          ...
          group.append("text").text(d.gender === 'M' ? '♂' : '♀').attr("x", -15).attr("y", 35).attr("fill", d.gender === 'M' ? '#556B2F' : '#ec4899').attr("font-size", "14px").attr("font-weight", "bold")
          group.append("text").text(`${d.age} Yrs`).attr("x", 10).attr("y", 35).attr("text-anchor", "middle").attr("fill", "#64748b").attr("font-size", "12px")
          
          if (d.is_committee) {
              group.append("text").text("★ Committee").attr("x", 0).attr("y", 65).attr("text-anchor", "middle").attr("fill", "#CDAA7D").attr("font-size", "10px").attr("font-weight", "bold")
          }
      }
